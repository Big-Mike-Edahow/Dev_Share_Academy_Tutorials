...
export default class GameManagerScript extends ScriptNode {
  ...
  #createPlayer() {
    ...
    const playerPosition = this.#tileToWorldPosition(
      playerTilePosition.x,
      playerTilePosition.y,
    );
    this.#playerPrefab = new PlayerPrefab(
      this.scene,
      playerPosition.x,
      playerPosition.y,
    );
  ...
  #createBoxes() {
    ...
    for (const tilePosition of boxTilePositions) {
      const boxPosition = this.#tileToWorldPosition(
        tilePosition.x,
        tilePosition.y,
      );
  ...
  #tileToWorldPosition(tileX, tileY) {
    const tileWorldX =
      tileX * this.wallTileLayer.tilemap.tileWidth +
      this.wallTileLayer.tilemap.tileWidth / 2;
    const tileWorldY =
      tileY * this.wallTileLayer.tilemap.tileHeight +
      this.wallTileLayer.tilemap.tileHeight / 2;

    const worldX =
      this.wallTileLayer.x + tileWorldX * this.wallTileLayer.scaleX;
    const worldY =
      this.wallTileLayer.y + tileWorldY * this.wallTileLayer.scaleY;
  ...
  #updateVisualPositions() {
    ...
    for (let i = 0; i < this.#boxPrefabs.length; i++) {
      ...
      const boxWorldPosition = this.#tileToWorldPosition(
        boxPosition.x,
        boxPosition.y,
      );
