...
export class SokobanGame {
  ...
  constructor(mapData) {
    ...
    this.#currentMap[this.#playerPosition.y][this.#playerPosition.x] =
      TILE_TYPES.FLOOR;
  ...
  #findBoxPositions() {
    ...
    for (let y = 0; y < this.#height; y++) {
      for (let x = 0; x < this.#width; x++) {
        if (
          this.#currentMap[y][x] === TILE_TYPES.BOX ||
          this.#currentMap[y][x] === TILE_TYPES.BOX_ON_GOAL
        ) {
          boxes.push({ x, y });
        }
      }
  ...
  movePlayer(direction) {
    ...
    if (!this.#canMoveToPosition(newX, newY)) {
      ...
      if (
        targetTile === TILE_TYPES.BOX ||
        targetTile === TILE_TYPES.BOX_ON_GOAL
      ) {
